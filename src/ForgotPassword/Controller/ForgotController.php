...
namespace ForgotPassword\Controller;
...
use ForgotPassword\Service\Password as PasswordService;
use ForgotPassword\Options\ForgotControllerOptionsInterface;
...
class ForgotController extends AbstractActionController
{
    ...
    public function forgotAction()
    {
        ...
        if ( $this->getRequest()->isPost() )
        {
            ...
            if ( $form->isValid() )
            {
                ...
                $vm->setTemplate('forgot-password/forgot/sent');
                return $vm;
            } else {
                $this->flashMessenger()->setNamespace('forgotpassword-forgot-form')->addMessage($this->failedMessage);
                return array(
                    'forgotForm' => $form,
                );
            }
    ...
    public function resetAction()
    {
        ...
        if ( $this->getRequest()->isPost() )
        {
            ...
            if ( $form->isValid() && $user !== null )
            {
                ...
                $vm->setTemplate('forgot-password/forgot/passwordchanged');
                return $vm;
            }
        }
    ...
    public function getPasswordService()
    {
        if (!$this->passwordService) {
            $this->passwordService = $this->getServiceLocator()->get('forgotpassword_password_service');
        }
    ...
    {
        if (!$this->forgotForm) {
            $this->setForgotForm($this->getServiceLocator()->get('forgotpassword_forgot_form'));
        }
    ...
    {
        if (!$this->resetForm) {
            $this->setResetForm($this->getServiceLocator()->get('forgotpassword_reset_form'));
        }
    ...
    public function getOptions()
    {
        if (!$this->options instanceof ForgotControllerOptionsInterface) {
            $this->setOptions($this->getServiceLocator()->get('forgotpassword_module_options'));
        }
        return $this->options;
    }
